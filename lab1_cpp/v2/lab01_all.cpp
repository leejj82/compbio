...
void print_olaps(olaps &list_of_olaps){
  ...
  for (int i=0;i<list_of_olaps.list.size();i++){
...
void read_from_olaps(olaps_2 &l_olaps){
  ...
  if (infile.is_open()) {
	...
	l_olaps.f_read_loc[read_1]=l_olaps.list.size();
	temp_olap.deleted=0;
  ...
  l_olaps.f_read_loc[num_of_reads]=l_olaps.list.size();

  for (i=0;i<l_olaps.exact.size();i++){//delete olaps connecting exact olaps
    for (j=0;j<l_olaps.list.size();j++){
...
void set_up_viable_edges(olaps_2 &l_olaps){
  ...
  for (i=0;i<l_olaps.exact.size();i++){
...
void insert_node_in_uni (node &current_node, olaps_2 &l_olaps,unitig &uni ){
  ...
  if (l_olaps.node[current_node.num].exact_copy_count>0){//take care of exact nodes
    node temp_node;
    for (int i=0;i<l_olaps.exact.size();i++){
...
void insert_bdry(vector<node> &ft_nodes,vector<node> & next_nodes, bool F){

  int i;
  
  ft_nodes=next_nodes;
  if(!F){//reverse complement case
    for (i=0;i<ft_nodes.size();i++)
...
void insert_boundary_nodes(vector<node> &next_nodes,unitig &uni, bool prior, bool F){
  ...
  if(prior)//previous nodes
    insert_bdry(uni.f_nodes,next_nodes,F);
  else
    insert_bdry(uni.t_nodes,next_nodes,F);
}
...
void find_nodes(node &current_node,olaps_2 &l_olaps,unitig &uni,bool prior){
  ...
  if(prior){//previous nodes case 
	...
	if (one_and_one(c_node, l_olaps,prior,F)){ //the current node has exactly one incoming edge and the previous node has exactly one outgoing edge
	  uni.nodes[uni.nodes.size()-1].offset=c_node.f_node[0].offset;
	...
	if (one_and_one(c_node, l_olaps,prior,RC)){ //the current node has exactly one incoming edge and the previous edge has exactly one outgoing edge
	  uni.nodes[uni.nodes.size()-1].offset=c_node.t_node[0].offset;
...
void insert_current_node_and_find_prior_and_posterior_nodes (node &current_node,olaps_2 &l_olaps,unitig &uni){
  ...
  for (int i=0;i<uni.nodes.size();i++)//compute unitig lengths
...
void find_a_unitig(int &starting_point, olaps_2 &l_olaps, unitigs &unis){
  ...
  if (connected_edges_to_the_current_node){ //there exists at least one edge connected to the node   
    ...
    insert_current_node_and_find_prior_and_posterior_nodes(current_node, l_olaps, unis.uni[unis.uni.size()-1]);
  }
...
void print_unis(unitigs &unis){
  ...
  for (i=0;i<unis.uni.size();i++){
    pFile<<"UNI  "<<setfill('0')<<setw(2)<<i+1<<" "<<setfill(' ')<<setw(5)<<unis.uni[i].nodes.size();//print the title
    pFile<<" "<<setfill(' ')<<setw(6)<<unis.uni[i].length<<"\n";
    for (j=0;j<unis.uni[i].nodes.size();j++){//print the rest
...
void find_unis_conn (node &Node, con &con,bool F){
  ...
  for (j=0;j<con.uni.size();j++){
    if(Node.num==con.uni[j].nodes[con.uni[j].nodes.size()-1].num){
      if(F==(Node.ori==con.uni[j].nodes[con.uni[j].nodes.size()-1].ori)){
	Node.num=j;
	Node.ori=F;
	break;
      }
...
void set_up_unis_connections(con &con){//find connections between unitigs and save the info in contig
  ...
  for (i=0;i<con.uni.size();i++){
    for (k=0;k<con.uni[i].f_nodes.size();k++)
      find_unis_conn(con.uni[i].f_nodes[k],con,F);
    for (k=0;k<con.uni[i].t_nodes.size();k++)
...
void set_up_uni_rc_and_node_location(con &con){ //calculate node locations in the unitigs and find and save reverse complement unitigs
  ...
  for( i=0;i<con.uni.size();i++){
    ...
    //take care of connected unitigs for each unitig in reverse complement unitig
    con.uni_rc[i].f_nodes=con.uni[i].t_nodes;
    for (j=0;j<con.uni_rc[i].f_nodes.size();j++){
      con.uni_rc[i].f_nodes[j].ori=!con.uni_rc[i].f_nodes[j].ori;
    }
    
    con.uni_rc[i].t_nodes=con.uni[i].f_nodes;
    for (j=0;j<con.uni_rc[i].t_nodes.size();j++){
    ...
    for (j=0;j<con.uni[i].nodes.size();j++)
    ...
    for (j=1;j<con.uni[i].nodes.size();j++){
...
int check_for_validity(vector<node> &new_unis_list,int & mate_ct_2,con &con, contig &contig){
  ...
  for (i=0;i<contig_unis_size;i++)//compute the number of reads in contig
    total_num_of_reads_in_contig+=con.uni[new_unis_list[i].num].nodes.size();
  ...
  for (i=0;i<contig_unis_size;i++){
    ...
    for (j=0;j<con.uni[new_unis_list[i].num].nodes.size();j++){//set the reads inside the contig
...
int mate_pair_check(con &con,node &uni1,node &uni2, int &distance){
  ...
  for (i=0;i<con.uni[uni1.num].nodes.size();i++){

    set_node(node1, uni1, con, i);//node1 is the first read for mate-pair check

    for (j=0;j<con.uni[uni2.num].nodes.size();j++){
...
bool iterate_for_finding_a_contig(con &con, vector<node> &unis_list, contig &contig){  
  ...
  for (k=0;k<(*uni)[last_uni.num].t_nodes.size();k++){
...
bool really_find_contig(con &con,contig &contig){
  ...
  for (i=0;i<con.uni.size();i++){//if an end of a unitig does not have connecting edges, then the unitig can be a boundary of a contig
    if(con.uni[i].f_nodes.size()==0 && con.uni[i].t_nodes.size()!=0){//we set it as the starting unitig
      first_uni.set(i,1,0);
      indicator=1;
      break;
    }
    else if(con.uni[i].f_nodes.size()!=0 && con.uni[i].t_nodes.size()==0){//we set it as the starting unitig
      first_uni.set(i,0,0);
      indicator=1;
      break;
    }
    else if(con.uni[i].f_nodes.size()==0 && con.uni[i].t_nodes.size()==0){//if there is no linked unitig
      indicator=-1;
      break;
    }
  }
