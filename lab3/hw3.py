...
for i in range(4):
    for j in range(4):
        for k in range(4):
            if [i,j,k] not in stop_codons_in_num:
                print >> overlap_file, temp[i]+temp[j]+temp[k], " ", frequencies[i][j][k]
...
for i in range(len(ORFs)):
    if ORFs[i][2]=='+':
        ...
        for j in range(codons_in_an_ORF):
...
print >> overlap_file, "gene count", gene_count
print >> overlap_file, "sensitivity", sensitivity
print >> overlap_file, "precision", precision
overlap_file.close() 
